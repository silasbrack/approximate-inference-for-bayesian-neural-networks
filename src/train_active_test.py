    ...
    for train, loaders, bs in zip(
        ...
        for i in range(5):
            ...
            loaders.append(
                data.DataLoader(
                    data.TensorDataset(
                        x[index], y[index].sub(2 * i).float().unsqueeze(-1)
                    ),
                    bs,
                    shuffle=True,
                    pin_memory=USE_CUDA,
    ...
    for train, loaders, bs in zip(
        ...
        c10 = CIFAR10(
            ...
            transform=tf.Compose(
                [tf.ToTensor(), tf.Normalize(C10_MEAN, C10_SD)]
            ),
        )
        loaders.append(
            data.DataLoader(c10, bs, shuffle=train, pin_memory=USE_CUDA)
        )

        c100 = CIFAR100(os.path.join(root, "cifar100"), train=train)
        unnormalized_data = (
            torch.from_numpy(c100.data).permute(0, 3, 1, 2).div(255.0)
        )  # convert images to torch arrays
        ...
        for i in range(5):
            ...
            if train:
                c100_means.append(
                    unnormalized_data_i.mean((0, 2, 3), keepdims=True)
                )
                c100_sds.append(
                    unnormalized_data_i.std((0, 2, 3), keepdims=True)
                )
            normalized_data_i = (
                unnormalized_data_i - c100_means[i]
            ) / c100_sds[i]
            ...
            loaders.append(
                data.DataLoader(
                    dataset_i, bs, shuffle=train, pin_memory=USE_CUDA
                )
            )
...
def main(root, dataset, inference):
    ...
    if dataset == "cifar":
        ...
        train_loaders, test_loaders = make_cifar_dataloaders(
            root, train_batch_size, test_batch_size
        )
        num_epochs = 60
    elif dataset == "mnist":
        net = FCNet()
    ...
    if inference == "mean-field":
        ...
        prior = tyxe.priors.IIDPrior(
            dist.Normal(0, 1), expose_all=False, hide_all=True
        )
        guide = None
    else:
        raise RuntimeError("Unreachable")
    ...
    for i, train_loader in enumerate(train_loaders, 1):
        ...
        for j, (test_loader, head_params) in enumerate(
            zip(test_loaders, head_state_dicts)
        ):
            net.Head.load_state_dict(head_params)
            err = sum(
                bnn.evaluate(x.to(DEVICE), y.to(DEVICE), num_predictions=8)[0]
                for x, y in test_loader
            )
            test_errors[i - 1, j] = err / len(test_loader.sampler)

        print("\t".join(["Error"] + [f"Task {j}" for j in range(1, i + 1)]))
        test_err_str = [f"{100 * e:.2f}%" for e in test_errors[i - 1, :i]]
        print("\t" + "\t".join(test_err_str))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", choices=["mnist", "cifar"], default="mnist"
    )
    parser.add_argument(
        "--inference", choices=["mean-field", "ml"], default="ml"
    )
